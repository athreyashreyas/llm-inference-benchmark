...
## Simplismart — Qwen3 4B on pricing page but absent from marketplace (confirmed)

- Stage: deploy
- What happened: After logging into Simplismart marketplace, Qwen3 4B was not available for deployment despite being listed on the public pricing page at $0.10/1M tokens. The smallest available Qwen3 variant in the marketplace was Qwen3 14B. Benchmark model was switched to Qwen3 14B to proceed.
- Agent impact: No — the agent had a confirmed model selection (Qwen3 4B) that turned out to be undeployable. Required human UI exploration to identify the correct available model. The fallback (Gemma 3 4B) was skipped in favour of staying in the Qwen3 family at 14B.
- Severity: 3
- Evidence: Simplismart pricing page lists "Qwen3 4B — $0.10/1M tokens". Marketplace login revealed Qwen3 14B as the smallest available Qwen3 deployment option. No Qwen3 4B endpoint found.
- Product impact: Pricing page and marketplace inventory are out of sync. A developer who selects a model based on the pricing page, configures their code with the expected model ID, and then attempts deployment will hit a silent failure — the model simply won't be there. This is a critical trust issue: pricing pages imply availability.
- Recommended fix: Pricing page should only list models with active deployable endpoints. Add a real-time availability indicator ("Available now" / "Coming soon") next to each model. Alternatively, auto-redirect to the closest available model with an explicit notice.
